...
def input_fields():
    def row1(params):
        ...
        with col1:
            params["doctype"] = st.selectbox("Type dokument", ["digibok", "digavis", "digitidsskrift", "digimanus", "digistorting", "kudos"], help="Dokumenttypene følger Nasjonalbibliotekets digitale typer",)
        with col2:
            lang = st.multiselect("Språk", ["nob", "nno", "dan", "swe", "sme", "smj", "fkv", "eng", "fra", "spa", "ger"], help="Velg fra listen", disabled=(params["doctype"] in ["digistorting"]),)
            params["langs"] = " AND ".join(list(lang)) if lang else None
        with col3:
    ...
    def row2(params):
        ...
        with cola:
            params["authors"] = st.text_input("Forfatter", "", help="Feltet blir kun tatt hensyn til for digibok", disabled=(params["doctype"] in ["digavis", "digistorting"]),)
        with colb:
            params["title"] = st.text_input("Tittel", "", help="Søk etter titler. For aviser vil tittel matche avisnavnet.", disabled=(params["doctype"] in ["digistorting"]),)

    def row3(params):
        cold, cole, colf = st.columns(3)
        with cold:
            params["fulltext"] = st.text_input("Ord eller fraser i teksten", "", help="Matching på innholdsord skiller ikke mellom stor og liten bokstav. Trunkert søk er mulig, slik at demokrat* vil finne bøker som inneholder demokrati og demokratisk blant andre treff",)
        with cole:
            params["ddk"] = st.text_input("Dewey desimaltall", "", help="Input matcher et deweynummer. For å matche hele serien føy til en `*`. Bruk OR for å kombinere: 364* OR 916*", disabled=(params["doctype"] in ["digistorting"]),)
        with colf:
            params["subject"] = st.text_input("Emneord", "", help="For å matche på flere emner, skill med OR for alternativ og AND for begrense. Trunkert søk går også — for eksempel vil barne* matche barnebok og barnebøker", disabled=(params["doctype"] in ["digistorting"]),)
...
def corpus_management(params):
    ...
    st.subheader("Lag korpuset og last ned metadata")  
    ...
    if df is not None and not df.empty:
        ...
        if df_defined:
            if st.download_button("Last ned data i excelformat", to_excel(df), filnavn, help="Åpnes i Excel eller tilsvarende",):
                pass
